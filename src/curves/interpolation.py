...
class YieldCurveInterpolator:
    ...
    def _build_interpolator(self):
        ...
        assert np.all(np.diff(self.maturities) > 0), "maturities doivent être strictement croissantes"
    ...
    def forward_rate(self, T1: float, T2: float) -> float:
        ...
        """
        # Eviter que T1 = T2
        assert T2 > T1, f"T2 doit être strictement supérieur à T1 (reçu: T1={T1}, T2={T2})"
...
if __name__ == "__main__":
    ...
    for i, T in enumerate(mats):
        ...
        logger.debug(f"Node {i}: T={T:.2f} | Z_boot={dfs[i]:.8f} | Z_interp={z_interp:.8f} | err={err:.2e}")
        assert err < 1e-10, f"Mismatch at {T}Y: err={err:.2e}"
        print(f"  {T:5.2f}Y : Z_boot={dfs[i]:.8f} | Z_interp={z_interp:.8f} | err={err:.2e}")
